...
Partial Class payment
    ...
    Protected Sub btnCode_Click(sender As Object, e As EventArgs)
        Dim code, id As String
        con.Close()
        code = "select * from tbl_order where order_id='" + txtcode.Text + "'"
        con.Open()
        cmd = New MySqlCommand(code, con)
        Dim dr As MySqlDataReader
        dr = cmd.ExecuteReader
        While dr.Read
            code = dr("order_status").ToString
        ...
        If String.IsNullOrEmpty(id) Or String.IsNullOrEmpty(id) Then
            lblErr.Text = "Invalid Order ID"

        ElseIf code <> "On process" Then
            lblErr.Text = ""
        Else
            divcode.Visible = True
            Dim constr As String = ConfigurationManager.ConnectionStrings("constr").ConnectionString
            Using cons As New MySqlConnection(constr)
                Using cmd As New MySqlCommand("select * from tbl_order where order_id='" + txtcode.Text + "'")
